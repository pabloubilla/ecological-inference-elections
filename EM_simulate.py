...
#@profile
def hit_and_run_matrix(n_m, p, b_m, G_size, I_size, samples = 1000, 
                       step_size = 100, p_distribution = False, 
                       load_bar = False, verbose = False, unique = True,
                       seed = 123):
...
def simulate_Z(n, p, b, samples = 1000, step_size = 100, p_distribution = False, 
               save = False, name = None, parallel = True, load_bar = False, 
               verbose = False, unique = True, seed = 123):
    ...
    if parallel:
        ...
        z_args = [(n[m], p, b[m], G_size, I_size, samples, step_size, p_distribution, load_bar, verbose, unique, seed*100 + m) for m in range(M_size)]
        with Pool() as pool:
    ...
    # Run sequentially
        ...
        for m in range(M_size):
            ...
            # n_m, p, b_m, G_size, I_size, samples = 100, step_size = 20, lambda_factor = 1.0, p_distribution = False, load_bar = False, verbose = False
            Z_m = hit_and_run_matrix(n[m], p, b[m], G_size, I_size, samples, step_size, p_distribution, load_bar, verbose, unique, seed) 
            Z.append(Z_m)
...
def EM_simulate(X, b, p_est = None, convergence_value = 0.0001, max_iterations = 100, 
                p_method = 'group_proportional', simulate = False, Z = None, unique = True,
                  instance = None, samples = 1000, step_size = 100, load_bar = True, verbose = True,
