...
def pre_process_EM(election_name, groups = ""):
    ...
    electors['GRUPO'] = electors['RANGO ETARIO'] # 
    if groups == '_HM':
...
def run_EM(election_name, groups = ""):
    ...
    #          "opt" : optimize groups
    #          "40"  : group by less and greater than 40
    #          "sex" : group by men and women
    #          "all" : consider all age-sex groups
    #          "single" : consider a single group
...
def add_pvalues(election_name, groups = ""):
    ...
    # read output pvalues (change to C if needed)
    file_dict_output_pvalues = f'{election_name}/output{groups}/{election_name}_output_pvalues.pickle'
    ...
    for d in lista_distritos:
        ...
        for c in circ_list:
            ...
            # path_circ = f'{election_name}/output{groups}/{d}/{c}'
            # if not os.path.exists(path_circ):
            #     os.makedirs(path_circ)
            df_circ = df_distrito[df_distrito['CIRCUNSCRIPCION ELECTORAL'] == c]
            df_circ.to_csv(f'{election_name}/output{groups}/{d}/{c}.csv')
            ...
            for l in loc_list:
                ...
                # df_local.to_csv(f'{election_name}/output{groups}/{d}/{c}/{l}.csv')
                # else:
                #     loc_list.remove(l)
        
             # guardar locales con mesas extrañas
            # with open(f'{election_name}/output{groups}/{d}/{c}/LOCALES.txt', "w") as output:
