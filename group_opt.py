...
def evaluate_all_combs_A_old(x, b, A, S = 20, std_thres = 0.05):
    ...
    for combination in group_combs_A:
        ...
        results = {}
        em = EM_mult(x, b_new, max_iterations = 10000,
                      dict_results = results, verbose = False, load_bar =False)
        ll = results['ll'] 
        if np.all(p_std < std_thres):
    ...
    return combination_list, ll_list, under_threshold, obj_func_list, log_obj_func_list, p_std_max_list

# new version: store all results in a dictionary
def evaluate_all_combs_A(x, b, A, S=20, std_thres=0.05, results_dict = {},
                         circ_name = ''):
    G = b.shape[1]
    group_combs_A = get_partitions(G, A)
    ...
    for combination in group_combs_A:
        ...
        em_results = {}
        EM_mult(x, b_new, max_iterations=10000, dict_results=em_results, verbose=False, load_bar=False)
        ll = em_results['ll']
...
if __name__ == '__main__':
    ...
    for c in circs:
        ...
        for a in range(G, A-1, -1):     
            ...
            evaluate_all_combs_A(x, b, a, S=20, std_thres = 0.05, results_dict = results_dict,
                                 circ_name=c)
